...
import ComeFromRightAnimation from "./../../../common/come-from-right-animation";

export default function ShownCards() {
  return (
    <article className={cn("flex flex-col gap-4 my-3", "w-full")}>
      <div className="container flex justify-start items-start max-h-[350px]  md:gap-10 gap-5">
        <ImageHover
          src={HomeProjects[0].image}
          alt={HomeProjects[0].title}
          width={1000}
          height={1000}
          className="md:basis-1/2 !max-h-[350px] overflow-hidden !flex-1"
        />

        <ComeFromRightAnimation className="flex flex-col items-start md:basis-1/2">
